use iced::{
    advanced::{
        ...
        Clipboard, Layout, Overlay, Shell, Widget, Renderer as _, text::Renderer as _,
    },
    alignment::{Horizontal, Vertical},
    event, keyboard, mouse, touch,
    widget::{button, column, container, row, slider, text, text_input, Button, Column, Container, Row, Slider, Space, Text},
    Border, Color, Element, Event, Length, Padding, Point, Rectangle, 
    Renderer, Shadow, Size, Vector,
};
use std::fmt::Write;

use crate::widget::helpers::overlay::Position;
...
impl OverlayState {
    ...
    fn update_from_rgb(&mut self) {
        let color = Color::from_rgb(self.red, self.green, self.blue);
        let (h, s, v) = rgb_to_hsv(color);
        self.hue = h;
        self.saturation = s;
        self.value = v;
...
impl<'a, Message: Clone> Overlay<Message, iced::Theme, Renderer> for ModernColorPickerOverlay<'a, Message> {
    fn layout(&mut self, _renderer: &Renderer, bounds: Size) -> Node {
        ...
        // Position the overlay near the button
        let mut position = self.position;
        
        // Keep it within bounds
        if position.x + size.width > bounds.width {
            position.x = bounds.width - size.width - 10.0;
        }
        if position.y + size.height > bounds.height {
            position.y = self.position.y - size.height - 30.0; // Show above button
        }
    ...
    fn draw(
        ...
        // Preset colors
        let preset_y = bounds.y + bounds.height - 70.0;
        let preset_size = 40.0;
        let preset_spacing = 10.0;

        for (i, color) in self.overlay_state.preset_colors.iter().enumerate() {
            let preset_x = bounds.x + 20.0 + (preset_size + preset_spacing) * i as f32;
        ...
        let add_preset_bounds = Rectangle {
            x: bounds.x + 20.0 + (preset_size + preset_spacing) * self.overlay_state.preset_colors.len() as f32,
            y: preset_y,
            width: preset_size,
            height: preset_size,
        };
...
impl<'a, Message: Clone> ModernColorPickerOverlay<'a, Message> {
    ...
    fn draw_sliders_tab(
        ...
        // Hex input
        let hex_y = bounds.y + 3.0 * spacing + 20.0;
        ...
        if let Some(position) = cursor.position_in(bounds) {
            ...
            if row < 8 && col < 12 {
                ...
                *self.is_open = false;
                ...
                // Grayscale row
                let gray_value = col as f32 / 11.0;
                let color = Color::from_rgb(gray_value, gray_value, gray_value);
...
// You might also want to add these trait implementations for better integration:
...
// Optional: Add methods to ColorButton for common use cases
impl<'a, Message> ColorButton<'a, Message> {
