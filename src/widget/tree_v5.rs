    ...
    fn update(
        ...
        match event {
            ...
            Event::Mouse(mouse::Event::ButtonPressed(mouse::Button::Left)) => {
                if let Some(cursor_pos) = cursor.position() {
                    // Try to handle as arrow click first
                    if self.handle_arrow_clicks(tree, layout, cursor_pos, shell) {
                        // Arrow was clicked - event consumed, don't delegate to content
                        return;
                    }
                    // If no arrow was clicked, fall through to content delegation
                }
            }
            _ => { }
        }
        ...
        for node in &mut self.nodes {
            if let (Some(content_layout), Some(content_tree)) = 
                (child_layouts.next(), tree.children.get_mut(tree_index)) {
                
                let content_bounds = content_layout.bounds();
                
                // Only forward mouse events if cursor is over this specific content
                let should_forward = match event {
                    Event::Mouse(_) => cursor.is_over(content_bounds),
                    _ => true, // Forward non-mouse events to all
                };

                if should_forward {
/*                     println!("Forwarding event {:?} to node {} (cursor over: {})", 
                        event, node.id, cursor.is_over(content_bounds)); */
                    ...
                    // For mouse events, stop after forwarding to the first content that handles it
                    if let Event::Mouse(_) = event {
                        if cursor.is_over(content_bounds) {
                            break; // Stop processing other nodes
                        }
                    }
                }
            }
    ...
    Renderer: iced::advanced::Renderer + iced::advanced::text::Renderer<Font = iced::Font>,
{
    ...
    fn handle_arrow_clicks(
        ...
        for node in &self.nodes {
            if let Some(row_layout) = child_layouts.next() {
                if self.check_arrow_click_in_row(node, row_layout, cursor_pos, tree, shell, 0.0 ) {
                    return true; // Arrow was clicked
                }
            }
        }
        false // No arrow was clicked
    }

    fn calculate_arrow_bounds(&self, row_bounds: Rectangle, indent_level: f32) -> Rectangle {
        let x_offset = indent_level * self.indent;
        let arrow_x = row_bounds.x + x_offset + ARROW_X_PAD;
        
        Rectangle::new(
            Point::new(arrow_x, row_bounds.y),
            Size::new(ARROW_W, ROW_HEIGHT),
        )
    }

    fn check_arrow_click_in_row(
        &self,
        node: &TreeNode<'a, Message, Theme, Renderer>,
        row_layout: Layout<'_>,
        cursor_pos: Point,
        ...
        if cursor_pos.y >= bounds.y && cursor_pos.y < bounds.y + ROW_HEIGHT {
            let arrow_bounds = self.calculate_arrow_bounds(bounds, indent_level);
            
            if !node.children.is_empty() && arrow_bounds.contains(cursor_pos) {
                let state = tree.state.downcast_mut::<State>();
                state.tree_state.toggle(node.id.clone());
