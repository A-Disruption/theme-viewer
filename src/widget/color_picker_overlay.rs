use crate::widget::{
    ...
    style,
    status::Status,
    helpers::color::{HexString, Hsv},
    helpers::overlay::Position,
};
    ...
    'b: 'a,
{
    ...
    fn on_event_rgba_color(
        ...
        // Create a vector to store the bar bounds for each color channel
        let mut bar_bounds = Vec::new();
        
        // Iterate through each RGBA row
        for _ in 0..4 {
            if let Some(row_layout) = rgba_color_children.next() {
                let mut row_children = row_layout.children();
                // Skip the label
                let _ = row_children.next();
                // Get the bar layout
                if let Some(bar_layout) = row_children.next() {
                    bar_bounds.push(bar_layout.bounds());
                }
            }
        }

        // Ensure we have all 4 bar bounds
        if bar_bounds.len() != 4 {
            return event::Status::Ignored;
        }

        let red_bar_bounds = bar_bounds[0];
        let green_bar_bounds = bar_bounds[1];
        let blue_bar_bounds = bar_bounds[2];
        let alpha_bar_bounds = bar_bounds[3];
...
fn block1<Message, Theme>(
    ...
    Message: Clone,
    Theme:
        style::Catalog + iced::widget::button::Catalog + iced::widget::text::Catalog,
{
    // ----------- Block 1 ----------------------
        ...
        style::Catalog + iced::widget::button::Catalog + iced::widget::text::Catalog,
{
    ...
    // Always redraw the sat/value area when dragging
    if color_picker.state.color_bar_dragged != ColorBarDragged::None {
        color_picker.state.sat_value_canvas_cache.clear();
    }
    ...
    let geometry = color_picker.state.sat_value_canvas_cache.draw(
        ...
        |frame| {
            ...
            frame.stroke(
                &Path::line(
                    Point::new(saturation_x, 0.0),
                    Point::new(saturation_x, frame.height()),
                ),
                stroke,
            );

            frame.stroke(
                &Path::line(Point::new(0.0, value_y), Point::new(frame.width(), value_y)),
                stroke,
            );
    ...
    // Always redraw the hue area when dragging
    if color_picker.state.color_bar_dragged == ColorBarDragged::Hue {
        color_picker.state.hue_canvas_cache.clear();
    }
