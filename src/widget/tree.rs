    ...
    Renderer: iced::advanced::Renderer + iced::advanced::text::Renderer<Font = iced::Font>,
{
                ...
                if let Some(position) = cursor.position() {
                    ...
                    if let Some(ref mut drag) = state.drag_active {
                        ...
                        if !found_target && position.y > tree_bounds.y && !branch_positions.is_empty() {
                            let (last_id, _, _, last_y, last_height, _, _) = 
    ...
    fn draw(
        ...
        renderer.with_layer(self.viewport, |renderer| {
            ...
            for &i in &self.dragged_indices {
                if i >= self.tree_handle.branch_content.len() {
                    continue;
                }
                
                let branch_height = if i < state.branch_heights.len() {
                    state.branch_heights[i].max(LINE_HEIGHT)
                } else {
                    LINE_HEIGHT
                };
                ...
                // Draw the branch background with all its decorations
                renderer.fill_quad(
                    renderer::Quad {
                ...
                // Draw the content for this specific branch
                // Calculate the position for the content
                let content_x = indent_x + ARROW_W + CONTENT_GAP;
                let content_bounds = Rectangle {
                    x: drag_bounds.x + content_x,
                    y: drag_bounds.y + y_offset,
                    width: state.branch_widths.get(i).copied().unwrap_or(100.0),
                    height: branch_height,
                };
                ...
                let translation = Vector::new(
                    ...
                    (drag_bounds.y + y_offset) - self.layout.bounds().y,
                );
